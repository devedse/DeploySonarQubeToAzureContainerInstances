$ACI_RESOURCE_GROUP='SonarResourceGroup'
$LOCATION='westeurope'
$ACI_STORAGE_ACCOUNT='sonarstorage'
$ACI_SQL_SERVER='sonarqubesql'
$ACI_SQL_USERNAME='dbadmin'
$ACI_SQL_PASSWORD='***************'
...
az storage file upload `
    --account-name $ACI_STORAGE_ACCOUNT `
	--share-name "sonarqubeconf" `
...
az group deployment create --name sonarqube --resource-group $ACI_RESOURCE_GROUP `
    ...
    --parameters storageAccountName=$ACI_STORAGE_ACCOUNT sqlUser=$ACI_SQL_USERNAME sqlServerName=$ACI_SQL_SERVER sqlPassword=$ACI_SQL_PASSWORD